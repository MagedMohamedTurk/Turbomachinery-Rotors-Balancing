...
class Model:
    ...
    def split(self, balancing_plane_index, holes_available, weights_available, max_number_weights_per_hole=None,
        ...
        if max_weight_per_plane:
            if isinstance(max_weight_per_plane, (float, int)):
                    const_splitting += [cp.sum(S.T @ weights_available, axis=0)
        ...
        W_equ_split = np.sum(weight_available_matrix * S)
        error = abs(W_equ_split - weight)
        # Estimate the error generated by using minmax instead of norm2 in the objective function
        problem_error = (Prob_S.value - error) / abs(weight)
        return df_splitting, S, tools.convert_cart_math(W_equ_split), error, problem_error
